...
pub impl syn::Ident {
    ...
    /// Converts the ident to snake_case without preserving its span.
    ///
    /// For the reason why span is not preserved, see the documentation of
    /// [`Self::to_pascal_case`] method.
    fn to_snake_case(&self) -> Self {
        quote::format_ident!("{}", self.to_string().to_snake_case())
    }
}
