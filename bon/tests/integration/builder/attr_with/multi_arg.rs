...
fn test_struct() {
    ...
    struct Sut<T: Clone + Default> {
        ...
        #[builder(with = |value: impl Into<IpAddr>, _value2: impl fmt::Debug| -> Result<_, Infallible> { Ok(value.into()) })]
    ...
    let builder = builder.try_impl_trait([127, 0, 0, 1], true).unwrap();

    assert_debug_eq(builder.build(), expect![[r#"
        Sut {
            required: 3,
            optional: Some(
                7,
