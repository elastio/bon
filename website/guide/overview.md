...
If you are new to the concept of builders or named function arguments and you don't know what problems they may solve for you, then check out the motivational [blog post](http://localhost:5173/bon/blog/how-to-do-named-function-arguments-in-rust).
...
The builders generated by `#[builder]` use the typestate pattern to make sure all required parameters are filled and the same setters aren't called repeatedly to prevent unintentional overwrites and typos. If something is wrong, a compile error will be created. There are no potential panics and `unwrap()` calls inside of the builder.
...
If you have members of type `String`, or `PathBuf`, and you need to set them to a hardcoded string literal, then you have to write `.to_owned()` or `.to_string()` or `.into()`.
...
See the ["Into conversions"](./into-conversions) page for more details.
...
However, feel free to skip the docs and just use the `#[builder]` macro in your code. It's designed to be intuitive, so it'll probably do the thing you want it to do already.
