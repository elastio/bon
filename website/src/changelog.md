...
- Strip the `r#` prefix from raw identifiers in `derive(Debug)` ([#402](https://github.com/elastio/bon/pull/402))
...
- Fix clippy::missing_docs_in_private_items triggering for start_fn members ([#389](https://github.com/elastio/bon/pull/389))
...
- Fix `clippy::wrong_self_convention` warning for fields with `is_` prefix etc. ([#349](https://github.com/elastio/bon/pull/349)). Thanks [@nicmue](https://github.com/nicmue) for the contribution!
...
- Fix visibility and `const` token order in code generated by `#[builder(const)]` ([#288](https://github.com/elastio/bon/pull/288))
