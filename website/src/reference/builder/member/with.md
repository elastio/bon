...
For members of type `Option<T>` without [`#[builder(transparent)]`](./transparent), the closure needs to return a value of type `T` or `Result<T>` for a fallible closure.
...
-   If there are `>=1` input parameters of types `T1, T2, ...`, then the maybe setter accepts `Option<(T1, T2, ...)>`
...
It's equivalent to writing either of the following:

This attribute can be used with custom and 3-rd party collections such as [`indexmap::IndexMap`](https://docs.rs/indexmap/latest/indexmap/map/struct.IndexMap.html), not only with the `std::collections`. The only requirement is for the collection type to match the following patterns.
...
Makes the setter accept the value of type `T` assuming the member is of type `Option<T>` with [`#[builder(transparent)]`](./transparent).

This attribute was added as a way to provide required setters for structs generated by [`prost`](https://docs.rs/prost/latest/prost/) from `proto3` syntax, that always use `Option<T>` internally even for required fields ([original issue comment](https://github.com/elastio/bon/issues/35#issuecomment-2426991137)).
